...
class Command(BaseCommand):
    help = "Scrape all product categories from Potaka (L1 → L2 → L3) and save to the database."

    def handle(self, *args, **options):
        site = Site.objects.filter(name="Potaka IT").first()
        if not site:
            self.stderr.write("Site 'Potaka IT' not found in the database.")
            return

        self.stdout.write(f"Fetching Potaka IT navigation from {site.url} ...")
        ...
        for l1_li in nav.find("ul", class_="responsive-menu").find_all("li", class_="drop-open", recursive=False):
            ...
            for l2_li in drop_1.find_all("li", class_="drop-open", recursive=False):
                ...
                for l3_li in drop_2.find_all("li", recursive=False):
        ...
        created, updated = save_categories(site.name, categories)
